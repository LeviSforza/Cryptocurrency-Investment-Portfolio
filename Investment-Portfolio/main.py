...
import investment_gui
import StockData.data_keys

APIS = APIs.API_list.APIS
PROFIT_TAX = 0.19
PORTFOLIO_PATH = 'StockData/wallet.json'
ASSET_TYPES = StockData.data_keys.ASSET_TYPES
DATA_KEYS = StockData.data_keys.DATA_KEYS
GUI = finalGui.Gui()
...
def addAssetToWallet():
    ...
    try:
        ...
        investment_gui.throwError('Error', 'Wrong asset data!')
...
def pushPortfolioToGui():
    ...
    try:
        ...
        investment_gui.throwError('Error', 'Wrong percent number!')
        percent = 10
    try:
...
def initializeGui():
    investment_gui.configButtonCommand(GUI.buttonAdd, addAssetToWallet)
    investment_gui.configButtonCommand(GUI.buttonShowPortfolio, pushPortfolioToGui)
    investment_gui.configButtonCommand(GUI.buttonShowWallet, pushWalletToGui)
    GUI.mainloop()
